...
// Reusable validation — used in both create and update
...
// Get All Records (with filters + pagination + search)
export const getAllRecords = async ({ page, limit, type, category, startDate, endDate, search }) => {
  ...
  if (search) {
    whereClause += ' AND (r.category LIKE ? OR r.description LIKE ?)';
  ...
  // Get records — JOIN with users to show who created each record
...
// Get Own Records (logged in user sees only their records)
...
export const getRecordById = async (id) => {
  const [rows] = await pool.query(
    `SELECT r.id, r.amount, r.type, r.category, r.date, r.description, r.created_at,
       r.updated_at, u.id   AS user_id, u.name AS user_name FROM financial_records r JOIN users u ON r.user_id = u.id WHERE r.id = ?`, [id]
  );
...
// Update Record 
export const updateRecord = async (id, { amount, type, category, date, description }) => {
